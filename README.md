...
- **Platform-level concerns only** — `charts/business-app/` generates the app-level Argo CD Applications and gitops-promoter CRs from each app's `.argocd/registry.yaml`; apps don't need to author those resources by hand.
...
charts/
  ...
  business-app/                     golden-path chart for hydrator+promoter services
    Chart.yaml
    values.yaml                     defaults (autoMerge, namespace, promoterNamespace, etc.)
    templates/
      application.yaml              generates one Argo CD Application per environment (sourceHydrator)
      git-repository.yaml           GitRepository CR
      promotion-strategy.yaml       PromotionStrategy CR (autoMerge configurable per env)
      argocd-commit-status.yaml     ArgoCDCommitStatus CR
```
...
> **Do not add app-specific resources here.** Gitops-promoter CRs and Argo CD Applications for a specific service are generated by `charts/business-app/` from the service's `.argocd/registry.yaml` — they should not be authored by hand anywhere.
